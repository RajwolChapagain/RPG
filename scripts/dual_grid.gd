...
var display_tileset_source_id: int = 0
var grass_placeholder_atlas_coord: Vector2i = Vector2i(0, 0)
var dirt_placeholder_atlas_coord: Vector2i = Vector2i(1, 0)
...
func _ready() -> void:
	for coord: Vector2i in %DataGrid.get_used_cells():
...
func set_tile(coords: Vector2i, atlas_coords: Vector2i) -> void:
	%DataGrid.set_cell(coords, 0, atlas_coords, 0)
...
func remove_deleted_display_tiles() -> void:
	var used_data_cells = %DataGrid.get_used_cells()
	for coord: Vector2i in %DisplayGrid.get_used_cells():
		for neighbor: Vector2i in NEIGHBORS:
			var data_neighbor_pos = coord - neighbor
			if data_neighbor_pos not in used_data_cells:
				%DisplayGrid.erase_cell(coord)
		
func calculate_display_tile(coords: Vector2i) -> Vector2i:
	var bot_right = get_data_tile(coords - NEIGHBORS[0])
	var bot_left = get_data_tile(coords - NEIGHBORS[1])
	var top_right = get_data_tile(coords - NEIGHBORS[2])
	var top_left = get_data_tile(coords - NEIGHBORS[3])
	
	return NEIGHBORS_TO_ATLAS_COORD[Vector4(top_left, top_right, bot_left, bot_right)]

func get_data_tile(coords: Vector2i) -> TileType:
	var atlas_coord = %DataGrid.get_cell_atlas_coords(coords)
