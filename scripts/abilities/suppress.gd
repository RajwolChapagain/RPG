class_name Suppress extends Ability

@export var suppress_effect: StatusEffect

func execute(_caster: Battler, target: Battler) -> void:
	target.APPLY_EFFECT(suppress_effect.duplicate(true))
...
func get_stats_dict() -> Dictionary[String, String]:
	return {
		...
		'Status Duration': "%d Ticks" % suppress_effect.duration_in_ticks,
	}
