class_name Stun extends Ability

@export var stun_effect: StatusEffect

func execute(_caster: Battler, target: Battler) -> void:
	target.APPLY_EFFECT(stun_effect.duplicate(true))
...
func get_stats_dict() -> Dictionary[String, String]:
	return {
		...
		'Status Duration': "%d Ticks" % stun_effect.duration_in_ticks,
	}
